      ...
      if (key === "color") {
        ...
        const colors = colorKeys.map((colorKey) => {
          ...
          const onClick = e => {
            if (e.target.checked){
              ...
              document.querySelector(`ul.detail-subcolor-visual#${e.target.value}`).style="display:none;"
          }
          return (
        ...
        const subcolors = colorKeys.map((colorKey) => {
          ...
          const detailColorList = detailColors.map((hex) => {
            ...
            const id=`sub_color=${hex}&`
            return (<label  htmlFor={id} className="color-selection-chip" key={hex} style={style}>
              <input onClick={onClick} className="color-selection-input-box" id={id}type="checkbox"/>
              </label>);
          });
